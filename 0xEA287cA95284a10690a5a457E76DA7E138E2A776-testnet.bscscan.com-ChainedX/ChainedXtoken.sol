...
//requirements:
// 1. only 3% sell tax - 1.98% staking, 0.51% treasury, 0.51% burn
// 100% tokens minted on deployment

// before deployment:
// TODO update staking and treasury addresses here

// after deployment:
// exclude pinksale, purchase, staking contract from sell fees after deployment

contract ChainedX is ERC20, Ownable {
    uint256 public buyFee = 0;
    uint256 public sellFee = 3;

    uint256 public maxBuyFee = 0;
    uint256 public maxSellFee = 3;

    uint256 public stakingShare = 66;
    uint256 public treasuryShare = 17;
    uint256 public burnShare = 17;

    // TODO update staking and treasury addresses here
    address public stakingAddress = 0x5aB9869069d3c32427Cb96178cbB4c783D00FBf2;
    address public treasuryWallet = 0x5aB9869069d3c32427Cb96178cbB4c783D00FBf2;
    ...
    constructor() ERC20("ChainedX", "CHDX") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            // check if this is the address of pancake swap router
            // bsc mainnet
    ...
    function updateBuyFees(uint256 _buyFee) external onlyOwner {
        require(_buyFee <= maxBuyFee, "Fees cannot be more than 0%");
        buyFee = _buyFee;
        emit BuyFeesUpdated(buyFee);
    }

    function updateSellFees(uint256 _sellFee) external onlyOwner {
        require(_sellFee <= maxSellFee, "Fees cannot be more than 3%");
        sellFee = _sellFee;
    ...
    function updateShares(
        ...
        require(
            _treasuryShare + _stakingShare + _burnShare == 100,
    ...
    function changeStakingAddress(address _stakingAddress) external onlyOwner {
        require(
            ...
            "Staking address is already that address"
        );
        require(
            isContract(_stakingAddress),
            "Staking address has to be a contract"
        );
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from]) {
            swapping = true;
            // TODO check the swap and transfer logic
        ...
        if (takeFee) {
            ...
            if (_totalFees > 0) {
                ...
                amount = amount - fees;
                uint256 treasuryTokens = (fees / 100
                    ) * treasuryShare;
                super._transfer(from, treasuryWallet, treasuryTokens);

                uint256 stakingTokens = (fees / 100
                    ) * stakingShare;
                super._transfer(from, stakingAddress, stakingTokens);
                uint256 burnTokens = (fees / 100
                    ) * burnShare;
