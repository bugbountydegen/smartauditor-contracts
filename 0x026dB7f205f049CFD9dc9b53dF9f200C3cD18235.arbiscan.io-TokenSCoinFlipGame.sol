...
contract TokenSCoinFlipGame is VRFConsumerBaseV2Plus, ReentrancyGuard {
    ...
    uint32 numWords = 1;
    uint32 public probabilityOfWin = 50;
    ...
    event BetResult(address indexed player, uint256 amount, uint256 requestId, bool win);
    event JackpotWon(address indexed player, uint256 amount, uint256 requestId);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) internal override {
        ...
        checkWin(requestId, player, betAmount, randomWords[0]);

        checkJackpot(requestId, player, betAmount, randomWords[0]);
    }

    function checkWin(uint256 requestId, address player, uint256 betAmount, uint256 randomWords) internal nonReentrant {
        ...
        if (win) {
            ...
            emit BetResult(player, reward, requestId, true);
        } else {
            // Игрок проиграл
            emit BetResult(player, betAmount, requestId, false);
        }
    }

    function checkJackpot(uint256 requestId, address player, uint256 betAmount, uint256 randomWords) internal nonReentrant {
        // Пополняем джекпот
    ...
    // Функция для изменения probabilityOfJackpot
    function setProbabilityOfJackpot (uint32 _probabilityOfJackpot) external onlyOwner {
        require(_probabilityOfJackpot >= 10, "Probability of jackpot must be more than 10");
        probabilityOfJackpot = _probabilityOfJackpot;
    }

    // Функция для изменения jackpotFee
    function setJackpotFee (uint32 _jackpotFee) external onlyOwner {
        require(_jackpotFee <= 15, "Jackpot fee must be less than 5%");
        jackpotFee = _jackpotFee;
    }

    // Функция для изменения contractFee
    function setContractFee (uint32 _contractFee) external onlyOwner {
        require(_contractFee <= 15, "Contract fee must be less than 10%");
