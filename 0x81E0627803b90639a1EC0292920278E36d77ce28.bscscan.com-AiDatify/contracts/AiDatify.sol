...
contract AiDatify is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint256 public constant V2_START_ID = 1425 + 10000000;
    AiDatify public constant AID_V1 = AiDatify(0x0B7836DA458C557e62d48460A0Ea86a5392AAFe5);
    ...
    function unstake(bytes calldata signature, uint256 stakeId, uint256 _amount, uint256 deadline) external {
        ...
        if (stakeId <= V2_START_ID && record.amount == 0) {
            (uint256 amount1, address owner1, bool status1) = AID_V1.stakeRecords(stakeId);
            require(!status1, "already unstake");
            require(!record.status, "already unstake");
            require(msg.sender == owner1, "invalid owner");
            stakeIds[msg.sender].add(stakeId);
            stakeRecords[stakeId] = StakeRecord({amount: amount1, owner: owner1, status: status1});
        }
    ...
    function stakeFor(uint256[] calldata _stakeIds) external onlyRole(OPERATOR_ROLE) {
        for (uint256 i = 0; i < _stakeIds.length; i++) {
            uint256 stakeId = _stakeIds[i];
            require(stakeId <= V2_START_ID, "invalid stakeId");
            require(stakeRecords[stakeId].amount == 0, "duplicate stakeId");
            require(!stakeRecords[stakeId].status, "invalid status");
            (uint256 amount1, address owner1, bool status1) = AID_V1.stakeRecords(stakeId);
            require(!status1, "already unstake");
            require(amount1 > 0, "invalid amount");
            require(stakeIds[owner1].add(stakeId), "duplicate stakeId");
            stakeRecords[stakeId] = StakeRecord({amount: amount1, owner: owner1, status: status1});
        }
    }
    ...
    function unstake(uint256 stakeId) internal {
        ...
        IStakePool(STAKE_POOL).redeemUnderlying(_amount);
        uint256 fee = (_amount * 5) / 100;
        IERC20(USDT).transfer(STAKE_POOL, fee);
        IERC20(USDT).transfer(msg.sender, _amount - fee);
        emit StakeLog(msg.sender, _amount, stakeId, 1, block.timestamp);
    }

    function getStakeIds(address addr) external view returns (uint256[] memory) {
        return stakeIds[addr].values();
    }
}
