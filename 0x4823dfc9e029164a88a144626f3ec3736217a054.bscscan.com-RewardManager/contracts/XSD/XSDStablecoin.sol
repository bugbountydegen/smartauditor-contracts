...
contract XSDStablecoin is BEP20Custom {
    ...
    function pool_price(PriceChoice choice) internal view returns (uint256) {
        ...
        uint256 _eth_usd_price = eth_usd_price();//(uint256(eth_usd_pricer.getLatestPrice())*PRICE_PRECISION)/(uint256(10) ** eth_usd_pricer_decimals);
    ...
    function eth_usd_price() public view returns (uint256) {
        return 1200000000;//(uint256(eth_usd_pricer.getLatestPrice())*PRICE_PRECISION)/(uint256(10) ** eth_usd_pricer_decimals);
    }
    //silver price
    //hard coded value for testing on goerli
    function xag_usd_price() public view returns (uint256) {
        return 21000000;//(uint256(xag_usd_pricer.getLatestPrice())*PRICE_PRECISION)/(uint256(10) ** xag_usd_pricer_decimals);
    }
